...
class SubscriptionController extends Controller
{
    ...
    public function submitCustomAlert(Request $request)
{
    $rules = [
        'email' => 'required|email|max:100',
        'custom_alert' => 'required|string|max:255', // Ensure custom alert title is provided
    ];
    $rules_messages = [
        'email.required' => __('E-mail address is required'),
        'custom_alert.required' => __('Alert title is required'),
    ];
    ...
    $subscription->search_title = $request->custom_alert; // Use custom alert title
    $subscription->save();

    return response()->json(['status' => true, 'msg' => 'Your alert has been successfully saved.']);
}
