        ...
        if (isset($user) && count($user->profileLanguages)):
            foreach ($user->profileLanguages as $language):
                ...
                $html .= '<tr id="language_' . $language->id . '">
						...
						<td><span>' . $language->getLanguageLevel('language_level') . '</span></td>
						<td align="right"><a href="javascript:;" onclick="showProfileLanguageEditModal(' . $language->id . ');" class="text text-dark"><i class="fas fa-pencil-alt"></i></a> <a href="javascript:;" onclick="delete_profile_language(' . $language->id . ');" class="text text-danger ms-2"><i class="fas fa-times"></i></a></td>
						</tr>';
            endforeach;
        endif;
    ...
    public function showApplicantProfileLanguages(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileLanguages)):
            foreach ($user->profileLanguages as $language):
                ...
                $html .= '<div class="langbox" id="language_' . $language->id . '">
						<h5>' . $language->getLanguage('lang') . '</h5>
						<p>' . $language->getLanguageLevel('language_level') . '</p></div>';
            endforeach;
        endif;
