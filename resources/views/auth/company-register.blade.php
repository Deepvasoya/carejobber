             ...
             <div id="employer" class="formpanel mt-0 tab-pane fade active">
                 ...
                 <form class="form-horizontal mt-3" method="POST" action="{{ route('company.register') }}">
                     ...
                     <div class="formrow{{ $errors->has('password_confirmation') ? ' has-error' : '' }}">
                         ...
                         <div class="formrow">
                             <label style="font-weight: normal; margin-bottom: 10px; display: block;">{{__('Do you have an invitation code?')}}</label>
                             <div style="margin-bottom: 10px;">
                                 <label style="font-weight: normal; margin-right: 15px; cursor: pointer;">
                                     <input type="radio" name="has_referral_code" value="yes" id="hasReferralYes" {{ (old('has_referral_code') == 'yes' || request('ref') || session('referral_code')) ? 'checked' : '' }} style="margin-right: 5px;">
                                     {{__('Yes')}}
                                 </label>
                                 <label style="font-weight: normal; cursor: pointer;">
                                     <input type="radio" name="has_referral_code" value="no" id="hasReferralNo" {{ (old('has_referral_code', 'no') == 'no' && !request('ref') && !session('referral_code')) ? 'checked' : '' }} style="margin-right: 5px;">
                                     {{__('No')}}
                                 </label>
                             </div>
                             <div id="referralCodeField" style="display: {{ (old('has_referral_code') == 'yes' || request('ref') || session('referral_code')) ? 'block' : 'none' }};">
                                 <input type="text" name="ref" id="referralCodeInput" class="form-control" placeholder="{{__('Enter invitation code')}}" value="{{ old('ref', request('ref') ?: session('referral_code')) }}">
                                 @if ($errors->has('ref')) <span class="help-block"> <strong>{{ $errors->first('ref') }}</strong> </span> @endif
                             </div>
                         </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    hasReferralYes.addEventListener('change', toggleReferralField);
    hasReferralNo.addEventListener('change', toggleReferralField);
