<div class="col-lg-3"> 
	...
	@else
	<a href="{{url('/build-resume')}}" class="btn"><i class="fa fa-file-text" aria-hidden="true"></i> {{__('Upload Your Resume')}}</a>
	@endif
	</div>
    ...
    <div class="sidebar" id="filterjobs">
        ...
        <!-- Jobs By State -->
        <div class="widget">
            <h4 class="widget-title">{{__('Jobs By Province/State')}}</h4>
            <ul class="optionlist view_more_ul">
        ...
        <div class="widget">
            ...
            <ul class="optionlist view_more_ul">
                @if(isset($facilityTypes) && count($facilityTypes))
                @foreach($facilityTypes as $industry_id => $industry_name)
                @php
                $checked = (in_array($industry_id, Request::get('industry_id', array())))? 'checked="checked"':'';
                @endphp
                <li>
                    <input type="checkbox" name="industry_id[]" id="industry_{{$industry_id}}" value="{{$industry_id}}" {{$checked}}>
                    <label for="industry_{{$industry_id}}"></label>
                    {{$industry_name}} <span>{{App\Job::countNumJobs('industry_id', $industry_id)}}</span>
                </li>
                @endforeach
                @endif
            </ul>
        ...
        <!-- Top Companies -->
        <div class="widget">
            <h4 class="widget-title">{{__('Jobs By Employer')}}</h4>
            <ul class="optionlist view_more_ul">
