            ...
            <div class="col-md-12"> 
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        ...
                        <div class="actions"> 
                            <a href="{{ route('create.industry') }}" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Industry</a>
                            <button type="button" id="bulkDeleteIndustries" class="btn btn-xs btn-danger" style="display:none;"><i class="fa fa-trash"></i> Bulk Delete</button>
                        </div>
                    </div>
                                ...
                                <table class="table table-striped table-bordered table-hover"  id="industryDatatableAjax">
                                    <thead>
                                        ...
                                        <tr role="row" class="heading">                                            
                                            <th><input type="checkbox" id="selectAllIndustries" /></th>
                                            <th>Language</th>
                                            <th>Industry</th>
                                            <th>Actions</th>
                                        </tr>
...
<script>
    ...
    $(document).on('change', '#selectAllIndustries', function() {
    ...
    $(document).on('change', '.checkboxes', function() {
        ...
        $('#selectAllIndustries').prop('checked', totalCheckboxes === checkedCheckboxes);
        toggleBulkDeleteButton();
    });
    ...
    function toggleBulkDeleteButton() {
        ...
        if (checkedCount > 0) {
            $('#bulkDeleteIndustries').show();
        } else {
            $('#bulkDeleteIndustries').hide();
        }
    }
    
    // Bulk Delete functionality
    $('#bulkDeleteIndustries').on('click', function() {
        var selectedIds = [];
        ...
        if (confirm('Are you sure you want to delete ' + selectedIds.length + ' selected item(s)?')) {
            $.ajax({
                url: '{{ route('bulk.delete.industries') }}',
                type: 'POST',
                data: {
                    ids: selectedIds,
                    _token: '{{ csrf_token() }}'
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        var table = $('#industryDatatableAjax').DataTable();
                        table.draw();
                        $('#selectAllIndustries').prop('checked', false);
                        toggleBulkDeleteButton();
                    } else {
