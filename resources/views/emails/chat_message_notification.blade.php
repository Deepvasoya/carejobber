    ...
    <tr>
        <td class="content-wrapper" style="padding-left:24px;padding-right:24px"><br>
            <div class="title" style="font-family: Helvetica, Arial, sans-serif; font-size: 18px;font-weight:400;color: #000;text-align: left;
                 padding-top: 20px;">{{__('Dear')}} {{ $recipient_name }},</div>
        </td>
    </tr>
    <tr>
                        ...
                        <table border="0" cellpadding="0" cellspacing="0" style="width:100%;">
                            <tr>
                                <td class="subtitle" style="font-family:Helvetica, Arial, sans-serif;font-size:14px;line-height:22px;font-weight:400;color:#333;padding-bottom:30px; text-align: left;">
                                    <br>
                                    {{__('You have received a new message from :sender on :site_name', ['sender' => $sender_name, 'site_name' => $siteSetting->site_name])}}.
                                    <br>
                                    <br>
                                    @if($message_type === 'text')
                                        <strong>{{__('Message')}}:</strong> {{ $message_preview }}
                                    @elseif($message_type === 'image')
                                        <strong>{{__('Message')}}:</strong> {{__('Image')}}
                                    @else
                                        <strong>{{__('Message')}}:</strong> {{__('File attachment')}}
                                    @endif
                                    <br>
                                    <br>
                                    <a href="{{ $chat_url }}" style="color: #fff;text-decoration: none;background: #f25a55; padding: 7px 10px;text-align: center;display: inline-block;margin-top: 20px;">{{__('View Message')}}</a>
                                    <br>
                                    <br>
                                </td>
                            </tr>
