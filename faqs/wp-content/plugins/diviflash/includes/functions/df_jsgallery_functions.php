...
function df_jsg_render_gallery_markup($options=[], $load_more_type = false){
    ...
    // Ensure $gallery is a string to avoid PHP deprecation when passing null to explode()
    $gallery = isset($gallery) && $gallery !== null ? (string) $gallery : '';

    // If no gallery provided return early (no images)
    if ( $gallery === '' ) {
        return '';
    }
