...
trait Df_Cpt_Taxonomy_Support {
    ...
    public function df_get_taxonomies($post_type) {
        ...
        if(!empty($taxonomies)) {
            ...
            foreach ( $taxonomies as $key => $taxonomy ) {
	            $tax_array[ $taxonomy->name ] = ! empty( $taxonomy->label ) ? $taxonomy->label : $taxonomy->name;
                $this->df_get_terms($taxonomy->name, $post_type);
            }
        } else {
     ...
     * @return null
     */
    public function df_get_terms($taxonomy, $post_type) {
        $this->term_settings[$post_type . '_terms_'. $taxonomy] = $this->df_term_settings($taxonomy, $post_type);
    }
    ...
    */
    ...
    public function get_multi_filter_terms($selected_taxonomy_list){
        ...
        if(!isset($this->df_taxonomies[$this->selected_post_type]) || !is_array($this->df_taxonomies[$this->selected_post_type])) return $main_value;
